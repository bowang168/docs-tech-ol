...
source_url: "https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user"
fetched: 2026-03-26
tags:
  - "oracle-linux"
  - "ol8"
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-Preface.html -->

## Preface

[Oracle Linux 8: KVM User's Guide](https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/) provides information about how to install, configure, and use the Oracle Linux KVM packages to run guest system on top of a bare metal Oracle Linux system. This documentation provides information on using KVM on a standalone platform in an unmanaged environment. Typical usage in this mode is for development and testing purposes, although production level deployments are supported. Oracle recommends that customers use Oracle Linux Virtualization Manager for more complex deployments of a managed KVM infrastructure.


---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-AboutOracleLinuxKVM.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-DescriptionoftheOracleLinuxKVMFeature.html -->
...
kernel version for KVM deployment. UEK users can also switch between
the KVM stacks as needed.  For more details about the virtualization packages
available for each Linux release, see [Virtualization Package Sources](kvm-VirtualizationPackageSources.html#kvm_VirtualizationPackageSources_Copy). For details about switching between
KVM stacks, see [Switch KVM Stacks](kvm-SwitchKVMStacks.html#kvm_SwitchKVMStacks "The following information describes how Oracle Linux 8 users can switch from the Default KVM stack to the Oracle KVM stack.The following information describes how Oracle Linux 8 users can switch from the Oracle KVM stack to the Default KVM stack.The following information describes how Oracle Linux 8 users can upgrade Oracle KVM stack packages to the latest version of the Oracle KVM stack by switching module streams.").
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-GuestOperatingSystemRequirements.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-SystemRequirementsandRecommendations.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-VirtualizationPackages.html -->
...
For installation instructions, see [KVM Host: Installation and Configuration](kvm-HostInstallationConfiguration.html "Install user-space packages, enable libvirt services, and validate that an Oracle Linux 8 system can host KVM virtual machines.").
...
| --- | --- |
| `virt` | Installs a collection of virtualization packages that include `libvirt`, `qemu-kvm`, and other dependencies. |
| `virtualization client` | Available for installation on GUI environment Oracle Linux systems. |
| `virtualization host` | Contains the minimum set of packages that are required for a virtualization host. |
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-VirtualizationPackageSources.html -->

## Virtualization Package Sources

Oracle distributes two sets of KVM user space packages, called
stacks. These KVM stacks define which versions of virtualization hypervisor packages
install and run on an Oracle Linux system.
...
If the system is running UEK, you can switch between stacks. For more details, see [Switch KVM Stacks](kvm-SwitchKVMStacks.html#kvm_SwitchKVMStacks "The following information describes how Oracle Linux 8 users can switch from the Default KVM stack to the Oracle KVM stack.The following information describes how Oracle Linux 8 users can switch from the Oracle KVM stack to the Default KVM stack.The following information describes how Oracle Linux 8 users can upgrade Oracle KVM stack packages to the latest version of the Oracle KVM stack by switching module streams.").
...
| --- | --- | --- | --- |
| Default KVM Stack | - Fully supported across all Oracle Linux kernels. - Offers maximum compatibility with RHCK and Red Hat Enterprise Linux. | `ol8_appstream`  (Enabled by default on all Oracle Linux 8 systems) | - The Default KVM stack can be installed without changing any repository   configuration. - These packages are released as part of the default DNF module: `virt` |
| Oracle KVM Stack | - Offers newer KVM features and functionality available in upstream packages. - Engineered to work with KVM features that are enabled in the latest releases of UEK. - Requires the latest versions of either UEK R6 or UEK R7. | `ol8_kvm_appstream` | - Before enabling this repository, you must remove all existing virtualization packages. - These packages are available as separate DNF module streams:   - `virt:kvm_utils`   - `virt:kvm_utils2`   - `virt:kvm_utils3`Note: Application streams have separate product support life cycles to   the base OS. Use the latest stream available whenever possible. See [Oracle Linux: Product Life Cycle Information](https://docs.oracle.com/en/operating-systems/oracle-linux/product-lifecycle/) for more information. - Some nonmodular packages, such as `virt-manager`,   `edk2`, `swtpm` and `libtpms` are   also included within this repository. |
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-HostInstallationConfiguration.html -->
...
1. **Optional:** If you require the Oracle KVM stack instead of the Default KVM stack, then
   [Enable Yum Repositories](kvm-EnableYumRepositories.html#kvm_EnableYumRepositories "To use any of the Oracle KVM stacks, first enable the ol8_kvm_appstream yum repository before installing the chosen application stream module.").

   Important:

   This step is only required
   when using the Oracle KVM Stack. The Default KVM Stack is available in
   the standard core system repositories.

   To identify the sources that include the Default KVM Stack or the Oracle KVM
   Stack, see [Virtualization Package Sources](kvm-VirtualizationPackageSources.html#kvm_VirtualizationPackageSources_Copy).
2. [Install Virtualization Packages](kvm-InstallVirtualizationPackages.html#kvm_InstallVirtualizationPackages "The following information describes how to install the virtualization packages on an Oracle Linux 8 system.").
3. [Validate Host System](kvm-ValidatingtheHostSystem.html#kvm-host-validate "Check that the system can host VMs.") to ensure that the system can host guest VMs.
4. **Optional:** On UEK systems only, [Switch KVM Stacks](kvm-SwitchKVMStacks.html#kvm_SwitchKVMStacks "The following information describes how Oracle Linux 8 users can switch from the Default KVM stack to the Oracle KVM stack.The following information describes how Oracle Linux 8 users can switch from the Oracle KVM stack to the Default KVM stack.The following information describes how Oracle Linux 8 users can upgrade Oracle KVM stack packages to the latest version of the Oracle KVM stack by switching module streams.") between the Default KVM stack and Oracle KVM stack.


---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-EnableYumRepositories.html -->

## Enable Yum Repositories

To use any of the Oracle KVM stacks, first enable the `ol8_kvm_appstream` yum repository before
installing the chosen
application stream module.

Ensure that the following are true before proceeding:

- The system meets the virtualization package requirements for the KVM
  application stream module that you're installing. See [KVM Host: System Requirements](kvm-SystemRequirementsandRecommendations.html#kvm-requirements) and
  [Virtualization Package Sources](kvm-VirtualizationPackageSources.html#kvm_VirtualizationPackageSources_Copy).
- Either UEK R6 or UEK R7 is
  installed and running on the system.
- You have administrator privileges.

This task is only required when using Oracle KVM stacks. The Default KVM stack is available
as an application stream module in the standard yum repositories
and can be installed without enabling any other repositories.

Tip:

If the Oracle KVM stack is already installed and you want to switch KVM stacks, see
[Switch KVM Stacks](kvm-SwitchKVMStacks.html#kvm_SwitchKVMStacks "The following information describes how Oracle Linux 8 users can switch from the Default KVM stack to the Oracle KVM stack.The following information describes how Oracle Linux 8 users can switch from the Oracle KVM stack to the Default KVM stack.The following information describes how Oracle Linux 8 users can upgrade Oracle KVM stack packages to the latest version of the Oracle KVM stack by switching module streams.").
...
1. Install or update the release package:
   ...
   ```
2. Enable the Oracle Linux 8 yum repository, by running the
...
After enabling the `ol8_kvm_appstream` repository, you can install the Oracle KVM Stack. See [Install Virtualization Packages](kvm-InstallVirtualizationPackages.html#kvm_InstallVirtualizationPackages "The following information describes how to install the virtualization packages on an Oracle Linux 8 system.").


---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-InstallVirtualizationPackages.html -->
...
- If installing the Oracle KVM stack:
  - The `ol8_kvm_appstream` yum repository must be enabled. See [Enable Yum Repositories](kvm-EnableYumRepositories.html#kvm_EnableYumRepositories "To use any of the Oracle KVM stacks, first enable the ol8_kvm_appstream yum repository before installing the chosen application stream module.") for instructions.
...
3. Install the KVM virtualization stack and other utilities: 
   ...
   - To install the Default KVM stack:
     ...
     ```
   - To install the Oracle KVM stack, first remove any existing virtualization
     packages and then install the module stream that you want to use. For example, to
     install the latest stream:
...
5. Ensure that the `libvirtd` service is running.
   ...
   Before you can create and manage KVM instances, the `libvirtd` service must be started and enabled. See
   [Manage the libvirtd Service](kvm-ManageLibvirtdService.html "The following information describes how to start, enable, and check the status of the libvirtd service.") for
   instructions.
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-ValidatingtheHostSystem.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-SwitchKVMStacks.html -->

## Switch KVM Stacks

You can switch from one KVM stack to another KVM stack, if required.

The KVM stacks define the versions of user space packages that you can use to manage virtual
machines on Oracle Linux. To take advantage of newer functionality
available in more recent user space packages, you must install UEK.

The Default KVM stack is compatible with both RHCK and UEK. The Default KVM stack is
available from the standard Oracle Linux package repositories. If you
require RHCK, you can only use the Default KVM stack.
...
Switching the KVM stack on an Oracle Linux system involves removing one
version of the KVM stack and then replacing it with another version of the KVM stack.
...
For instructions on how to switch KVM stacks on Oracle Linux
8 systems with UEK, see these topics:

- [Switch Default Stack to Oracle KVM Stack](kvm-SwitchKVMStacks.html#kvm_SwitchFromDefaultToOracleStack "The following information describes how Oracle Linux 8 users can switch from the Default KVM stack to the Oracle KVM stack.")
- [Switch Oracle KVM Stack to Default Stack](kvm-SwitchKVMStacks.html#kvm_SwitchFromOracleToDefaultStack "The following information describes how Oracle Linux 8 users can switch from the Oracle KVM stack to the Default KVM stack.")
- [Upgrade the Oracle KVM Stack](kvm-SwitchKVMStacks.html#task_ofk_fx5_lcc "The following information describes how Oracle Linux 8 users can upgrade Oracle KVM stack packages to the latest version of the Oracle KVM stack by switching module streams.")

Note:

For details about the KVM stacks and their sources, see [Virtualization Package Sources](kvm-VirtualizationPackageSources.html#kvm_VirtualizationPackageSources_Copy).

### Switch Default Stack to Oracle KVM Stack

The following information describes how Oracle Linux
8 users can switch from the Default KVM stack
to the Oracle KVM stack.

Ensure that the following are true before proceeding:

- The Oracle Linux
  8 system has the Default KVM stack
  packages installed.
...
Guest instances that were created using the default
KVM stack might not be compatible and might not start after switching to the Oracle KVM
stack.

Follow these steps to switch from the
Default KVM stack to the Oracle KVM Stack.
...
1. Remove any packages from the existing default `virt` module stream: 
   ...
   Oracle Linux 8 virtualization stacks are managed by switching
   between application streams for the `virt` module. For more information
   about using DNF modules and application streams, see [Oracle Linux: Managing Software on Oracle
   Linux](https://docs.oracle.com/en/operating-systems/oracle-linux/software-management/)
2. Reset the `virt` module state so that it's neither enabled nor disabled: 
   ...
   ```
3. Enable the `ol8_kvm_appstream` yum repository.

   For more information, see [Enable Yum Repositories](kvm-EnableYumRepositories.html#kvm_EnableYumRepositories "To use any of the Oracle KVM stacks, first enable the ol8_kvm_appstream yum repository before installing the chosen application stream module.").
4. Enable the appropriate module and stream. For example, to use the latest packages: 
   ...
   ```
5. Perform any necessary package upgrade or downgrade operations to handle dependencies
...
The following information describes how Oracle Linux
8 users can switch from the Oracle KVM stack to
the Default KVM stack.

Ensure that the following are true before proceeding:

- The Oracle Linux
  8 system has the Oracle KVM stack  module and stream installed.
- The host system is running the latest UEK R6 or UEK R7 .
- You have administrator privileges.

Caution:

Guest instances that were created using the Oracle
KVM stack might not be compatible and might not start after switching to the Default KVM
stack.

Follow these steps to switch from the
Oracle KVM stack to the Default KVM stack.

1. Remove any of the packages from the existing Oracle KVM stack module stream, for
   example: 
...
3. Enable the `virt` module and stream: 
   ...
   ```
4. Perform any necessary package upgrade or downgrade operations to handle dependencies
   for the enabled module and stream:
...
5. Install the base packages from the `virt` stream: 
   ...
   ```
6. Disable the `ol8_kvm_appstream` yum repository:

   ```
   sudo dnf config-manager --disable ol8_kvm_appstream
   ```

   If you don't intend to use the Oracle KVM stack, disabling the associated repository prevents installation of packages from that repository in the future and ensures compatibility between installed KVM components.

### Upgrade the Oracle KVM Stack

The following information describes how Oracle Linux
8 users can upgrade Oracle KVM stack packages to the latest
version of the Oracle KVM stack by switching module streams.

Ensure that the following are true before proceeding:

- You have an Oracle Linux 8 system with an earlier Oracle KVM stack stream
  installed.
- The host system is running the latest UEK R6 or UEK R7.
- You have administrator privileges.
...
Guest instances that were created using an earlier
Oracle KVM stack stream might not be compatible and might not start after switching to the
later version of the Oracle KVM stack.

Follow these steps to switch from an earlier Oracle KVM stack stream to the latest version
of the Oracle KVM stack.

1. Remove any of the packages from the existing Oracle KVM stack.

   For example, to switch from the `virt:kvm_utils2` module stream:
   ...
   about using DNF modules and application streams, see [Oracle Linux: Managing Software on Oracle
   Linux](https://docs.oracle.com/en/operating-systems/oracle-linux/software-management/).
2. Reset the `virt` module state so that it's neither enabled nor
   disabled: 

   ```
   sudo dnf module reset virt -y
   ```
3. Enable the latest module and stream.

   For example, to enable `virt:kvm_utils3`:

   ```
   sudo dnf module enable virt:kvm_utils3 -y
   ```
4. Perform any necessary package upgrade or downgrade operations to handle
   dependencies required for the enabled module and stream:

   ```
   sudo dnf --allowerasing distro-sync
   ```
5. Install the later version of the Oracle KVM stack packages.

   For example, to install the packages from `virt:kvm_utils3`:
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-ManageLibvirtdService.html -->

## 3 Manage the `libvirtd` Service

The following information describes how to start, enable, and
check the status of the `libvirtd` service.

Ensure that you have the following:

- Virtualization packages installed on host system. See [Install Virtualization Packages](kvm-InstallVirtualizationPackages.html#kvm_InstallVirtualizationPackages "The following information describes how to install the virtualization packages on an Oracle Linux 8 system.") for details.
...
3. To check the status of the libvirtd service, run:
   ...
   The output indicates whether the `libvirtd` daemon is running, as shown
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-KVMUsage.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-CreatingaNewVirtualMachine.html -->
...
- The KVM host has been prepared, as described in [KVM Host: Installation and Configuration](kvm-HostInstallationConfiguration.html "Install user-space packages, enable libvirt services, and validate that an Oracle Linux 8 system can host KVM virtual machines.").
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-CloningVirtualMachines.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-StartingandStoppingVirtualMachines.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-virsh-console.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-start-stop-remove.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-virsh-migrate-offline.html -->
...
- The `libvirtd` service must be enabled on
  both hosts. See
  [Manage the libvirtd Service](kvm-ManageLibvirtdService.html "The following information describes how to start, enable, and check the status of the libvirtd service.").
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-BaseOperations.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-config_vm_watchdog.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-ConfiguringaVirtualMachinewithaVirtualTrustedPlatformModule.html -->
...
You can add a vTPM to an existing Oracle Linux 8 KVM. When you enable vTPM, the KVM files are encrypted but not the disks. Although, you can choose to add encryption explicitly for the KVM and its disks.
...
3. Perform these steps to add the vTPM settings to the guest OS XML configuration
   ...
   2. Update the guest OS XML configuration file to include the vTPM security properties. 
      ...
      When creating a KVM instance for the first time on Oracle Linux 8, you can also use the `virt-install` command `--tpm` option to specify the TPM emulated device information at installation time. For example:
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-network-topicgroup.html -->
...
To achieve high availability networking for any VMs, you must first configure a network bond on the host system. For details on how to set up network bonding, see Network Bonding in [Oracle Linux 8: Setting Up Networking](https://docs.oracle.com/en/operating-systems/oracle-linux/8/network/).
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-WorkingWithStorageforKVMGuests.html -->
...
- [Creating a Storage Volume from XML](kvm-WorkingWithStorageforKVMGuests.html#unique_997849628)
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-WorkingWithMemoryandCPUAllocation.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-KnownIssuesforOracleLinuxKVM.html -->
...
- [Downgrade Stream Conflict Error Messages](kvm-KI_issue_34623368.html#topic_bz1_bpj_z5b)


---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/8/kvm-user/kvm-KI_issue_34623368.html -->

## Downgrade Stream Conflict Error Messages

Known Issue: Package conflict errors appear when downgrading
`virt:kvm_utils2` to a previous application stream version.

Description:

Beginning with version 20744+e9607200 of the `virt:kvm_utils2` application
stream, new packages were added. If you try to downgrade to a previous version of the
`virt:kvm_utils2` application stream that doesn't contain the extra
packages, the downgrade process fails with several package conflict error messages. This
issue is the result of a limitation in DNF for handling dependencies in application
streams.

Workaround:

To resolve this issue, you must remove the existing packages, reset the `virt:kvm_utils2` application stream, enable the older version of the `virt:kvm_utils2` application stream and then reinstall the packages. See [Switch Default Stack to Oracle KVM Stack](kvm-SwitchKVMStacks.html#kvm_SwitchFromDefaultToOracleStack "The following information describes how Oracle Linux 8 users can switch from the Default KVM stack to the Oracle KVM stack.") for steps to remove existing packages, resetting the application stream and then installing packages.
