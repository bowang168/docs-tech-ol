...
source_url: "https://docs.oracle.com/en/operating-systems/oracle-linux/10/oscap"
fetched: 2026-03-26
tags:
  - "oracle-linux"
  - "ol10"
...
Oracle Linux 10 Using OpenSCAP for Security Compliance
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/oscap/oscap-Preface.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/oscap/oscap-WhatIsSCAP.html -->
...
The Oracle Linux installer also provides options to install the OS to match a specific security profile or policy as defined by the XCCDF profiles available in the `scap-security-guide` package. By applying a policy during installation, you can ensure the system is compliant when it begins operation. See [Oracle Linux 10: Installing Oracle Linux](https://docs.oracle.com/en/operating-systems/oracle-linux/10/install/) for more information.
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/oscap/oscap-SCAPPackages.html -->
...
For information about SCAP package features and other changes in Oracle Linux 10, see the release notes in the [Oracle Linux 10 documentation](https://docs.oracle.com/en/operating-systems/oracle-linux/10/).
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/oscap/oscap_information_and_reference.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/oscap/oscap-CheckingComplianceWithOSCAP.html -->
...
```
oscap info /usr/share/xml/scap/ssg/content/ssg-ol10-ds.xml
```
...
Stream: scap_org.open-scap_datastream_from_xccdf_ssg-ol10-xccdf.xml
Generated: (null)
Version: 1.3
Checklists:
	Ref-Id: scap_org.open-scap_cref_ssg-ol10-xccdf.xml
		Status: draft
		Generated: date
		Resolved: true
		Profiles:
...
The `ssg-ol10-ds.xml` data stream file contains information about where to download OVAL definitions so that evaluations can audit against the most recent version of these definitions.
...
```
oscap info --profile xccdf_org.ssgproject.content_profile_hipaa /usr/share/xml/scap/ssg/content/ssg-ol10-ds.xml
```
...
Profile
	...
	Description: The HIPAA Security Rule establishes U.S. national standards to protect individuals's electronic personal health 
            ...
            protected health information.  This draft profile configures Oracle Linux 10 to the HIPAA Security Rule identified for securing
...
2. Run a scan using that profile.
   ...
   The `--fetch-remote-resources` option lets OSCAP connect to the internet to download remote resources that are required for the XCCDF profile evaluation. Or, you can use the `--local-files` option for OSCAP to access those resources at the specified path. The `ssg-ol10-ds.xml` data stream file includes a reference to the remotely hosted OVAL definitions that can verify whether a system is patched appropriately.
...
4. Review the resulting XML file.
   ...
   ...
   Test Results:
   	Result ID: xccdf_org.open-scap_testresult_xccdf_org.ssgproject.content_profile_anssi_bp28_minimal
   	Source benchmark: /usr/share/xml/scap/ssg/content/ssg-ol10-ds.xml
   	Source profile: xccdf_org.ssgproject.content_profile_anssi_bp28_minimal
...
1. Create a full security guide for a system based on an XCCDF profile.
   ...
   ```
   sudo oscap xccdf generate guide --profile xccdf_org.ssgproject.content_profile_anssi_bp28_minimal \
   /usr/share/xml/scap/ssg/content/ssg-ol10-ds.xml > /var/www/html/security_guide.html
   ```
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/oscap/xccdf_remediation.html -->
...
In earlier releases of Oracle Linux, users could select a compliance profile during OS installation using the "Security Profile" option in the Oracle Linux installer, automatically applying certain hardening configurations at install-time. This feature isn't available in Oracle Linux 10 and later.

However, some compliance profiles might require system changes (such as specific disk partitioning or kernel parameter settings) that can only be applied during installation. These changes can't be remediated automatically after installation by using XCCDF profile remediation scripts. For full compliance with certain profiles, ensure that you review profile requirements and, if required, manually apply configuration changes during the installation process.
...
sudo oscap xccdf eval --profile standard \
                --remediate /usr/share/xml/scap/ssg/content/ssg-ol-ds.xml
```
...
oscap xccdf generate fix --profile profile id --fix-type bash \
                --output all-remediations.sh /usr/share/xml/scap/ssg/content/ssg-ol10-ds.xml
```
...
oscap xccdf generate fix --profile profile id --fix-type blueprint \
                --output blueprint.toml /usr/share/xml/scap/ssg/content/ssg-ol10-ds.xml
```


---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/oscap/auditing_for_vulnerabilities_by_using_oval_definitions.html -->
...
1. Download the definition files.
   ...
   For example, to download the consolidated OVAL definitions for all ELSA patches for Oracle Linux 10, run:

   ```
   wget https://linux.oracle.com/security/oval/com.oracle.elsa-ol10.xml.bz2
   ```
2. Extract the consolidated definition files, if required.
   ...
   ```
   bzip2 -d com.oracle.elsa-ol10.xml.bz2
   ```
3. Run a scan.
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/oscap/oscap-ScanContainerImagesandContainers.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/oscap/oscap-ScanOfflineFileSystems.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/oscap/scanning_remote_systems.html -->
...
Although it might be possible to connect as the root user on a remote system directly over SSH, we recommend not doing this. Always use `oscap-ssh` with the `--sudo` option and configure an appropriate user on the remote system for this task. See [Oracle Linux 10: Setting Up System Users and
Authentication](https://docs.oracle.com/en/operating-systems/oracle-linux/10/userauth/) for more information.