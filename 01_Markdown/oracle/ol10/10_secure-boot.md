...
source_url: "https://docs.oracle.com/en/operating-systems/oracle-linux/10/secure-boot"
fetched: 2026-03-26
tags:
  - "oracle-linux"
  - "ol10"
...
Oracle Linux 10 Working With UEFI Secure Boot
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/secure-boot/sboot-Preface.html -->

## Preface

[Oracle Linux 10: Working With UEFI Secure Boot](https://docs.oracle.com/en/operating-systems/oracle-linux/10/secure-boot/) provides background and other related
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/secure-boot/sboot-OverviewofSecureBoot.html -->
...
:   These keys can be built into the OS and they check signatures as kernel modules are
    ...
    the public key is built into the kernel.
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/secure-boot/sboot-ToolsandApplicationsforAdministeringSecureBoot.html -->
...
The`mokutil` utility can be used to review and update UEFI SBAT revocation
...
The most
current UEFI DBX files and the `dbxtool` utility that's used to manage the
DBX revocation list are in the `fwdup` package and updates are present within
each later release of this package on the Oracle Linux yum server. The DBX files are
prefixed by `DBXUpdate` and found in the
`/usr/share/fwupd/remotes.d/vendor/firmware/` directory.
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/10/secure-boot/sboot-SigningKernelModulesforUseWithSecureBoot.html -->
...
2. [Signing the Kernel Image](sboot-SigningKernelModulesforUseWithSecureBoot.html#sb-mod-sign-kernel-image)
3. [Updating the MOK Database](sboot-SigningKernelModulesforUseWithSecureBoot.html#sb-mod-sign-mok)
...
If you can't or don't want to enroll a signing certificate as described in [Updating the MOK Database](sboot-SigningKernelModulesforUseWithSecureBoot.html#sb-mod-sign-mok), you can enroll a specific kernelâs hash into the MOK database. This lets you load that particular kernel under Secure Boot, even without a certificate.
...
1. Extract the kernel's hash.
   ...
   ```
   pesign -h -i /boot/vmlinuz-$(uname -r)  | awk '{print $1}' > vmlinuz.hash
   ```

   Note:

   The `pesign` tool returns a hash value and the path to the kernel module. In version 116 of the `pesign` tool packaged with Oracle Linux 10, the fields are returned in a different order to earlier versions.
...
1. [Signing the Kernel Module](sboot-SigningKernelModulesforUseWithSecureBoot.html#topic_fjh_lts_x5b)
...
The `.machine` keyring is linked to the
