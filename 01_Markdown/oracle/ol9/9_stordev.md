...
source_url: "https://docs.oracle.com/en/operating-systems/oracle-linux/9/stordev"
fetched: 2026-03-26
tags:
  - "oracle-linux"
  - "ol9"
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/stordev/stordev-Preface.html -->

[Oracle Linux 9: Managing Storage
Devices](https://docs.oracle.com/en/operating-systems/oracle-linux/9/stordev/) provides information about storage device management,
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/stordev/stordev-UsingDiskPartitions.html -->
...
Alternatively, you can use the `cfdisk` utility, which is a text-based,
graphical version of `fdisk`.
...
changes and manages events related to changes. Storage devices, partitions,
and file systems are all allocated unique identifiers that the
`udev` subsystem can read and use to automatically
configure device mappings that you can use to identify the device or
partition that you intend to work use. Device mappings for storage devices
managed by udev are stored in `/dev/disks` and you can
identify a device by various identifiers, including the unique partition
UUID, file system UUID, or the partition label.
...
uses the disk partitions that were created in [Creating Partitions](stordev-UsingDiskPartitions.html#fdisk-create-partition) as
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/stordev/stordev-ImplementingSwapSpaces.html -->
...
1. Create the swap partition by using either
   ...
   - If using `fdisk`, create the
     ...
     [Creating Partitions](stordev-UsingDiskPartitions.html#fdisk-create-partition).
   ...
   - If using `parted`, specify
     ...
     [Partitioning Disks by Using parted](stordev-UsingDiskPartitions.html#diskpart-parted).
...
on `/dev/sda2` and a 1GB swap file,
`/swapfile`. The `Priority`
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/stordev/stordev-RecommendationsforSolidStateDrives.html -->
...
system, see Creating Swap Files on a Btrfs File System in [Oracle Linux 9: Managing Local File Systems](https://docs.oracle.com/en/operating-systems/oracle-linux/9/fsadmin/).


---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/stordev/stordev-WorkingWithLogicalVolumeManager.html -->
...
an abstraction layer, LVM provides you the capability to by which you can configure physical
and logical volumes. With LVM, you obtain data redundancy as well increased I/O performance.
...
way as you would a physical device. If a file system on a logical volume becomes full with
data, you can increase the volume's capacity by using free space in the volume group. You can
then grow the file system, if the file system supports that capability. Physical storage
devices can be added to a volume group to further increase its capacity.
...
To display information about physical volumes, use the
`pvdisplay`, `pvs`, and
`pvscan` commands.
...
Archiving volume group âmyvgâ metadata (seqno 0).
...
To merge a snapshot with its original volume, use the
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/stordev/stordev-WorkingWithSoftwareRAID.html -->
...
exports the RAID volumes as LUNs, or in software by the operating system. The Oracle Linux kernel uses the multiple device (MD) driver to support software
RAID to create virtual devices from two or more physical storage devices. MD enables you to
organize disk drives into RAID devices and implement different RAID levels.
...
1. Ensure that you have created a sufficient number of physical volumes in a volume group to
   accommodate your LVM RAID logical volume requirements. For more information about creating
   physical volumes and volume groups, see [Working With Logical Volume Manager](stordev-WorkingWithLogicalVolumeManager.html#using-lvm).
...
3. Run the `lvcreate` command to create the LVM RAID device.
   ...
   - [RAID Level 0 (Striping) LVM Examples](stordev-WorkingWithSoftwareRAID.html#topic_bkq_hl3_h1c)
   - [RAID Level 1 (Mirroring) LVM Examples](stordev-WorkingWithSoftwareRAID.html#topic_mb4_jl3_h1c)
   - [RAID Level 5 (Striping with Distributed Parity) LVM Examples](stordev-WorkingWithSoftwareRAID.html#topic_zwq_ll3_h1c)
   - [RAID Level 6 (Striping with Double Distributed Parity) LVM Examples](stordev-WorkingWithSoftwareRAID.html#topic_mjm_nl3_h1c)
   - [RAID Level 10 (Striping of Mirrored Disks) LVM Examples](stordev-WorkingWithSoftwareRAID.html#topic_t3h_pl3_h1c)
4. Create the filesystem you want on your device. For example, the following
   command creates an ext4 file system on a RAID 6 logical volume.
   ...
   ```
5. Consider persisting the logical volume by editing your `/etc/fstab` file.
   ...
   file, see [Automatic Device Mappings for Partitions and File Systems](stordev-UsingDiskPartitions.html#topic_afq_fs3_stb)
   .
6. In the event that a device failure occurs for LVM RAID levels 5, 6, and 10, ensure that
...
`myvg` volume group. The stripesize of four kilobytes is the size of
...
now running the `myvg-mylvraid0` RAID 0 logical volume. Additionally,
each instances of `myvg-mylvraid0` is a subvolume included in another
...
volumes are now part of the `myvg-mylvraid1` RAID 1 logical volume.
Additionally, each instance of `myvg-mylvraid1` includes subvolume pairs
for data and metadata. Each data subvolumes are labled
`myvg-mylvraid1_rimage_0` and
...
volumes are now part of the `myvg-mylvraid5` RAID 5 logical volume.
Additionally, each instance of `myvg-mylvraid5` includes subvolume pairs
for data and metadata. Each data subvolumes are labelled
`myvg-mylvraid5_rimage_0`, `myvg-mylvraid5_rimage_1`,
and `myvg-mylvraid5_rimage_2`. Each metadata subvolumes are labelled
...
are now part of the `myvg-mylvraid6` RAID 6 logical volume. Additionally,
each instance of `myvg-mylvraid6` includes subvolume pairs for data and
...
Additionally, each instance of `myvg-mylvraid10` includes subvolume pairs
for data and metadata. Each data subvolumes are labelled
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/stordev/stordev-UsingEncryptedBlockDevices.html -->
...
The device mapper supports the encryption of block devices
through the `dm-crypt` device driver. Data on
these devices are accessible at boot time only with proper
credentials. `dm-crypt` encrypts disk
partitions, RAID volumes, and LVM physical volumes, regardless
of their contents.
...
3. Create an entry for the encrypted volume in
   ...
   This entry causes the operating system to prompt you for the
   passphrase at boot time.
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/stordev/stordev-WorkingWithLinuxIOStorage.html -->
...
storage devices. iSCSI encapsulates SCSI commands in IP network packets to support data
transfer over long distances and sharing of storage by client systems. iSCSI uses the existing
IP infrastructure and doesn't require the purchase and installation of fiber-optic cabling and
interface adapters that are needed to implement Fibre Channel (FC) storage area networks.
...
[Figure 7-1](stordev-WorkingWithLinuxIOStorage.html#about-linux-io__iscsi-fig) shows a
simple network where several iSCSI initiators can access the shared storage that's attached to
an iSCSI target.

Figure 7-1 iSCSI Initiators and an iSCSI Target Connected via an IP-based Network

A hardware-based iSCSI initiator uses a dedicated iSCSI HBA. Oracle Linux supports iSCSI initiator functionality in software. The kernel-resident device driver uses
...
For more information about LIO, see
[http://linux-iscsi.org/wiki/Main\_Page](https://linux-iscsi.org/wiki/Targetcli).
...
10. Configure the access rights for logins by initiators.
    ...
    The demonstration mode is inherently insecure. For
    information about configuring secure authentication modes,
    see
    [http://linux-iscsi.org/wiki/ISCSI#Define\_access\_rights](https://linux-iscsi.org/wiki/ISCSI#Define_access_rights).
...
2. Use a discovery method, such as
   ...
   This command also starts the `iscsid`
...
3. Display information about the targets that are now stored in
...
Any discovered LUNs remain available across reboots provided that the target continues to
serve those LUNs and you don't log the system off the target.
...
If the LUNs that are available on an iSCSI target change, use
the `iscsiadm` command on an iSCSI initiator to
update the entries in its discovery database. The following
example assumes that the target supports the
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/stordev/stordev-WorkingWithnvme_fs.html -->
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/stordev/stordev-UsingMultipathingforEfficientStorage.html -->
...
[Figure 9-1](stordev-UsingMultipathingforEfficientStorage.html#about-device-multipath__dmpath-fig)
...
[Working With the Multipathing Configuration File](stordev-UsingMultipathingforEfficientStorage.html#multipath-conf-file).
...
Alternatively, you can view the settings in
`/etc/multipath.conf`.
...
4. Edit `/etc/multipath.conf` as required.
   ...
   For details, see [Working With the Multipathing Configuration File](stordev-UsingMultipathingforEfficientStorage.html#multipath-conf-file).
