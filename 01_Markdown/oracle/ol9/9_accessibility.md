...
source_url: "https://docs.oracle.com/en/operating-systems/oracle-linux/9/accessibility"
fetched: 2026-03-26
tags:
  - "oracle-linux"
  - "ol9"
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/accessibility/accessibility-Preface.html -->

The [Oracle Linux 9: Accessibility User's Guide](https://docs.oracle.com/en/operating-systems/oracle-linux/9/accessibility/) describes the accessibility
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/accessibility/accessibility-WorkingWithAccessibilityFeaturesinEnterpriseLinux.html -->
...
configuring the accessibility features that are included in Oracle Linux 9.

### Selecting a Desktop Version

Oracle Linux9 offers both the Standard and
...
3. At the bottom right corner of the screen, click the cogwheel.
...
doesn't provide any accessibility features.

### Accessing Assistive Technologies

In Oracle Linux, the assistive technologies are listed on the Accessibility
...
- Accessing through the
  ...
  1. On the top bar, click
     Activities.
  2. On the search field, type `accessibility`.
  3. From the list of options on the left panel, select Accessibility.
- Accessing through the command line
  ...
  2. On the terminal window, type:
     ...
     ```
     gnome-control-center accessibility
     ```
  3. From the list of options on the left panel, select Accessibility.
...
- Accessing through the
  ...
  1. On the top bar, click Applications, select Other, then select Settings.
...
Oracle Linux provides an Accessibility Menu, which enables you to
access and configure accessibility features without the need to open the Accessibility panel. This menu
is disabled by default. To make the menu available, open the Accessibility panel and set the Always
Show Accessibility Menu switch to On. Toggling this switch makes the Accessibility
Menu icon become permanently visible on the top bar of the desktop.
...
Enabling selected features in the Accessibility panel also automatically enables quick access,
even if the Always Show Accessibility Menu switch is disabled. However, in
...
Toggling the Always Show Accessibility Menu switch ensures that quick access
is available permanently regardless of whether assistive features are enabled or not.


---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/accessibility/accessibility-OverviewofAccessibilityFeatures.html -->

In the GNOME desktop, features for aiding users with impairments are configured from the Accessibility
panel.
...
- Pointing & Clicking
  ...
  Locate Pointer
  :   Enables you to find the position of the pointer on the screen by pressing the
      control key.
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/accessibility/accessibility-CustomizingAccessibilityFeatures.html -->
...
9 system by default. When enabled, the Orca
...
5. Enable the screen reader by using either the Accessibility panel or, if available,
...
3. Customize the magnifier according to your preferences.
   ...
   Click each tab to configure the different options on those tabs.
4. (Optional) Toggle the Zoom
   switch to ON to use the feature immediately.

   You can also enable the magnifier later through the Accessibility Menu icon on the
   desktop's top bar.
...
Except for Enable By Keyboard, the settings can be enabled through the Accessibility
Menu icon on the desktop's top bar.
...
---
<!-- page: https://docs.oracle.com/en/operating-systems/oracle-linux/9/accessibility/accessibility-UsingBraille.html -->
...
4. Enable the screen reader by using either the Accessibility panel or, if available,
   the quick access icon on the desktop's top bar.
